...
|---|---|---|
| [jalbarrang/umalator-global](https://github.com/jalbarrang/umalator-global) | **The engine we vendor** (`src/sim/`) | Ships (a) web app, (b) simulation engine, (c) data tooling incl. `pnpm run db:fetch` to pull `master.mdb` and extraction scripts. **GPL-3.0** — see §4. **Pin: v0.27.0 (`484539f5`) — the engine re-platformed to a Rust core compiled to WebAssembly; we followed 2026-07-10 (one pin now covers engine + data). See [docs/provenance.md §1](docs/provenance.md) + [docs/data-refresh-runbook.md §1](docs/data-refresh-runbook.md).** |
| [kachi-dev VFalator](https://kachi-dev.github.io/uma-tools/umalator-global/) ([repo](https://github.com/kachi-dev/uma-tools)) | Reference behavior; wit-variance mechanics (downhill mode, rushed, proc chance) | Direct umalator fork with fixes; spurt-rate accuracy cross-checked vs packet data. The "VFalator" line; sibling of our jalbarrang pin (both descend from alpha123 + Kachi). |
...
| [TheCing/uma-tools](https://github.com/TheCing/uma-tools) ("Moomoolator", [live](https://umalator.app)) | **Reference/inspiration only — do NOT re-pin to it** (surveyed 2026-06-24) | Sibling umalator fork (alpha123 → kachi-dev/VFalator → TheCing). Adds **UI/UX only** (bilingual JP+EN, mobile-first, dark/light, PNG "Uma Card" export, OCR import) — no engine-physics edge over our pin. **Preact** (clashes with our React 19) + no versioned engine. The right upgrade lever is bumping our jalbarrang pin, not adopting this. |
| [daftuyda/UmaTools](https://github.com/daftuyda/UmaTools) ([live](https://daftuyda.moe)) | **UX/feature inspiration (no race sim)** — surveyed 2026-06-24 | General training companion: in-browser **OCR event-choice helper** (Tesseract.js), rating calculator, **formula-scored** SP/skill optimizer (rating / Team-Trials modes — M2/M4 UX ideas), deck builder, skill library. **License says GPL-3.0 but unconfirmed (GitHub API `license: none`) — read the actual LICENSE before reusing any code.** Pulls data from **GameTora** (off-limits — don't borrow its data path). Re-implement patterns (vanilla JS → our React), don't lift. |
...
| [ayaliz/hakuraku](https://github.com/ayaliz/hakuraku) (**MIT**, [live](https://hakuraku.moe/)) | **M1 inheritance-math harvest source** (read 2026-06-24 → [docs/hakuraku-m1-harvest.md](docs/hakuraku-m1-harvest.md)). Shipped affinity/pair-affinity (confirms our `src/core/affinity.ts`) + the new bits to port: **spark proc chance** (individual-affinity model `min(100, base·(1+aff/100))` + proc-odds DP → new `src/core/sparkChance.ts`) and a **factor/transfer optimizer** (`rankParents`). Also a packet-capture (CarrotJuicer) race-replay viewer + meta analytics (those parts NOT portable to our static planner; ⚠️ external packet capture has ToS/account-risk). Re-derive + validate vs umamily.moe / Ice's sheet, don't transcribe. **Also a strong UX reference** for the design-system/fidelity track (data-dense panels, drill-down modals, ECharts data-viz, transparent data browser) — built on **React-Bootstrap + ECharts**, not a custom token system; MIT so the component code is readable, not just the rendered UI. |
...
| **uma.moe inheritance API** ([`/api/v3/search`](https://uma.moe/api/v3/search) + `/api/v4/user/profile/{id}`) | **Rental-parent search (M1, feasibility verified 2026-06-14)** — the one live, CORS-open, residual-spec-matching rental-parent API (per-color spark filters, "need N of" groups, server-side affinity via `player_chara_id`, `follower_num=999`=open-to-friend; spark int = `factorId×10+star`; profile endpoint drills into a trainer's `veterans[]`). **Auth-gated** (per-user `X-API-Key` or Turnstile browser-proof; no anonymous tier) → integrate via a **user-supplied key** (email operator Tunnelbliick), never a baked-in key (P2). Backend open-source but **AGPL-3.0** → port the affinity algo with attribution, don't vendor. **P3:** a matching record ≠ a borrowable rental (follow-then-borrow 3/day, DB lag 2–7d, private/slot-full profiles). |
| JP CM history (GameWith/Kamigame per-CM pages) | JP prior per CM (manual curation) |

### Timeline / schedule / banner / patch sources (Module 3 — added 2026-06-14; tiers: official ▸ datamined ▸ prediction)
| Resource | Tier | Use for |
|---|---|---|
| [Official Global News](https://umamusume.com/news/) (Cygames) | **official (authority)** | THE confirmation source for every CM date + race conditions, banner/scout window, balance patch. Client-side rendered → store the stable `/news/<id>/` permalink as each entry's citation (e.g. `/news/790/` Gemini, balance `/news/100040/`); body needs headless/search-snippet. [@umamusume_eng](https://x.com/umamusume_eng) + official Discord = faster human-watch mirror |
| [Game8 Upcoming Banners](https://game8.co/games/Umamusume-Pretty-Derby/archives/537125) | prediction | **Primary scrapeable forecast feed** — char+support+est-date, multi-year, clean static HTML (models pace, not balance) |
| [Game8 monthly schedule + CM guide](https://game8.co/games/Umamusume-Pretty-Derby/archives/541155) | prediction/confirmed | Near-term confirmed Global CM dates/windows/track; monthly pages mint a new archive ID each month (discover off wiki index) |
| SoulEC Timeline sheet — [CSV export](https://docs.google.com/spreadsheets/d/e/2PACX-1vQ1bJUx-NQHJvwYE3Jcs9UP5aFVdWwoPVZc5Z7GN3oUGyB7-Rl8YZ48y-lJEvFWTGIrQmOa-yTehPkK/pub?output=csv) | prediction | The "Timeline sheet" (§3) — quantifies the **~1.3–1.6× JP→Global acceleration**. Pull via `pub?output=csv`/gviz, NOT the pubhtml landing page |
| [Phoenix Timeline sheet](https://docs.google.com/spreadsheets/d/1ClwQpAPpVJDnHWYAkbAsk7k9DfvkQ8CJJvjQy3jg4Wc/edit) | prediction | Secondary — QoL/system/scenario rollout (partial "patches"); CSV export |
| [uma.guide CM schedule](https://uma.guide/cm-schedule/) | aggregator | **Most scrape-friendly per-CM track data** (course/ground/weather/skill-zones, ~44 CMs); FIXED order — pair with a date predictor |
| [GameTora Foresight Timeline](https://gametora.com/umamusume/foresight-timeline) + [CM Viewer](https://gametora.com/umamusume/events/champions-meeting) + [Gacha History (en)](https://gametora.com/umamusume/gacha/history?server=en) | prediction / aggregator — **cite, don't scrape (ToS)** | Gold-standard predictor + per-CM track detail; Gacha History (server=en) = authoritative PAST Global banner dates → calibrate the pace multiplier |
| umalator `db:fetch` + `sync:data` (vendored) → live Global master.mdb + [GameTora upcoming overlay](https://github.com/jalbarrang/umalator-global) | **datamined (server='jp' preview, P4)** | ToS-clean route: mdb = ground truth for *released* Global content (course geometry, skill params for patch-diffing); GameTora overlay "upcoming" flag = ahead-of-Global (mark preview, never mix into Global calc) |
| [snep.pw anni](https://pages.snep.pw/uma/anni-skill-changes/) / [anni2](https://pages.snep.pw/uma/anni2-skill-changes/) skill-changes | aggregator | **Patches**: what Global inherits at each anniversary rebalance (pair with official balance bulletins for the date) |
| [Victoria Frontier Discord](https://discord.com/invite/victoriafrontier) (~28.5k) | datamined — human intel | Real-time leak/datamine discussion; human monitoring only, not a feed (no scraping) |

> Patches have **no good predictive feed** — predictors model banner/CM *pace*, not balance divergence. Confirmed patches: official `/news/100040/`-pattern bulletins; "what's coming": anni changelogs + umalator data-version diffs. **Avoid:** GameTora direct scraping (ToS — deep-link only), uma.moe/umamusume.run (JS SPAs), SimpleSandman mirrors (deprecated 2025-10-30).
...
| **M2-input feasibility** (researched 2026-06-14): [Magody/Umaplay](https://github.com/Magody/Umaplay) (PaddleOCR+YOLO, reads the *live* Global skill screen — automation, ToS/ban-risk), UmaTools (GPL-3.0 OCR preprocess + fuzzy-matcher pattern), [receipt_factor](https://lt900ed.github.io/receipt_factor/) (JP-hardcoded factor-OCR stitcher, no license) | **The post-run skill-screen state (SP / learnable list / cost / hint) is NOT file/API-extractable on Global** — packet tools (CarrotJuicer family) are DMM-only; Steam adds **CrackProof** kernel anti-cheat; UmaExtractor only has the *finished-uma* record (→ M1, not M2). So **M2 input = carry-from-M4 + manual**; OCR deferred (confidence-gated, UmaTools pattern). |
