...
- ✅ **M1.2 — plan picker (reuses the inventory card).** *Landed 2026-06-26.* The left-column "pick your plan" surface is the **shared `PlanInventoryCard`** we already use on the planner page — pick a saved plan there; the active plan **glows** (`is-uma1`) like the inventory rows. Wired to the shared active-plan context (uma1 slot) + the shared apply-track-on-load setting. (First built as a bespoke search-based card; **reworked per user feedback** to reuse the real inventory card — search bar dropped, exact inventory visual incl. the selected glow border. The bespoke `UmaPlanCard`/`umaPlanApt` were removed.) **880 tests.** Plan: [m1-2-uma-plan-card](superpowers/plans/2026-06-26-m1-2-uma-plan-card.md) (search-card version; superseded by the inventory-card rework).
...
| **M1 Inheritance workbench** | **🟧 active** | **from the 2026-06-25 handoff; built card-by-card M1.0–M1.8 (see section). M1.0 logic + M1.1 shell/route + M1.2 plan picker (reuses inventory card) done (880 tests); M1.3 (plan-targets card) next. Importer/data follow-on.** |
