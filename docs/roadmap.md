...
- 🟩 **M1.7 — "Obtainable vs. wishlist" card.** *Done (2026-07-02); one data gate left.* Coverage **matrix** (`Innate ‖ Parent | G.parent ‖ Hint | Chain | Random`, real per-member inherit-%, uncovered badge) + **bonus** list + Matrix/Coverage toggle. `greenSparkReconcile.ts` + `g1Saddle.ts` + `coverageMatrix.ts` + `CoverageMatrixCard` wired in `InheritancePage`. **Family-aware `○≡◎` matching** across all columns (gold/× separate); **Innate = the uma's non-unique kit from the engine bundle** (`loadInnateSkillsByUmaId`, white/gold — obviated the gametora innate pipeline). **Data-gated pending:** populate `data-overrides/g1_saddle_ids.json` + in-src copy with real G1 saddle ids → win-bonus goes live (currently 0, safe). **1163 tests.**
...
| **M1 Inheritance workbench** | **🟧 active** | **from the 2026-06-25 handoff; built card-by-card M1.0–M1.8 (see section). On main: M1.0–M1.6 (incl. M1.4 inheritance card + Star-Tracks picker + M1.6 support-card pool). **M1.7 coverage matrix landing via this PR:** family-aware `○≡◎` matching, Innate kit from the engine bundle, career training-event column + event popup, icon-spec sort (uniques-top / gold-above-white), ○-normalized white sparks, unified collapsible bonus table, click-to-open skill-detail popup; one data gate left (populate G1 saddle ids). Next: M1.8 target-spark card.** |
