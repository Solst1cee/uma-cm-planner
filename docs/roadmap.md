...
- ✅ **M1.2 — "Your uma plan" card + inventory popover.** *Landed 2026-06-26.* Left-column card: portrait + name/epithet + plan-relevant pink aptitude chips (`umaPlanAptChips`), with an **inventory-icon button** (not "Change") that pops the **shared `PlanInventoryCard`** as a **dismiss-on-outside** popover. Clicking a row loads that plan as current (uma1) and closes the popover; the **1/2 slot badges are hidden** (new `hideSlotBadges` prop on `PlanInventoryCard`, default off → M4 unchanged). (Iterated twice on feedback: bespoke search-card → full inventory-card-in-column → this popover form.) **887 tests.** Plan: [m1-2-uma-plan-card](superpowers/plans/2026-06-26-m1-2-uma-plan-card.md) (early version; UI iterated past it).
...
| **M1 Inheritance workbench** | **🟧 active** | **from the 2026-06-25 handoff; built card-by-card M1.0–M1.8 (see section). M1.0 logic + M1.1 shell/route + M1.2 uma-plan card (inventory-icon popover) done (887 tests); M1.3 (plan-targets card) next. Importer/data follow-on.** |
