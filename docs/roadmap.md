...
| **M1 Inheritance workbench** | **🟧 active** | **from the 2026-06-25 handoff; built card-by-card M1.0–M1.8 (see section). On main: M1.0–M1.7 (M1.7 coverage matrix merged via PR #38). **M1.8 target-spark card landing via PR #40** — the workbench's last placeholder card; every M1 card is now built. Remaining M1 work: M1.4b rental builder + search-link (consumes M1.8's `buildTargetSpark` contract), the G1 saddle-id data gate, M1.0 `wonRaces` import, then Plans 3–5 (roster migration, search-builder + compare, pedigree view).** |
