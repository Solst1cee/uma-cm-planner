...
- ✅ **M1.2 — "Your uma plan" card.** *Landed 2026-06-26.* Left-column card: portrait + name/epithet + the plan-relevant pink aptitude chips (surface/distance/strategy keys graded by the uma's `baseAptitudes`, via pure `umaPlanAptChips`) + a **Change** button revealing the reused `SearchPicker` over the trained-uma list (swaps `uma1Plan.umaId` via `setPlan`). Presentational `UmaPlanCard` is provider-free (portrait + picker icons passed as props). **885 tests.** Plan: [m1-2-uma-plan-card](superpowers/plans/2026-06-26-m1-2-uma-plan-card.md).
...
- 🟧 **M1.5 — "Deck" card.** *Built — in PR #11 → `feat/m1-inheritance-workbench` (not yet merged), 2026-06-26.* 6-slot drag-drop deck (HTML5 DnD `text/card-id`) + per-slot 4-diamond LB stepper + remove/clear, in CM-planner card-head grammar. **Deck state is browser-local & plan-independent** (`scb_deck` / `scb_deck_active` / `scb_profiles`) — NOT `CmPlan.lockedDeckSlots`, NOT per-plan. Templates use an **autosave combobox** (no Save/Load buttons): name = active template (autosaves into it live), caret dropdown loads / **New** / per-row **×** delete; seeds a **Default** on first load, **New survives reloads**, typing an existing name switches (no overwrite), unnamed work is preserved as `Untitled`. The drag *source* / "+ Add" arrive with M1.6 (`addCardToDeck` seam + drop target built). **939 tests.** Spec/plan: [2026-06-26-m1-5-your-deck-card](superpowers/plans/2026-06-26-m1-5-your-deck-card.md).
- ✅ **M1.6 — "Support cards" card.** *Built — 2026-06-26.* Pool panel in the center column: **Icon / Art / Plot** views, filters (Rarity / Type / Skill / free-text), Matches · Effect sort (hidden in Plot). euophrys training-power score vendored (`src/vendor/uma-tiers/`, MIT) — per-type scoring, `DEFAULT_SCENARIO` = MANT + bondPerDay:10, weights persisted to `scb_score_weights`. Cards without a euophrys row show `—` / omitted in Plot (P3 honesty). Tiles draggable + `+ Add` fills the deck. **Known gap:** Stats filter + per-tile stat-line deferred. **967 tests.** Spec/plan: [m1-6 sdd](.superpowers/sdd/).
...
| **M1 Inheritance workbench** | **🟧 active** | **from the 2026-06-25 handoff; built card-by-card M1.0–M1.8 (see section). M1.0 logic + M1.1 shell/route + M1.2 uma-plan card + M1.3 plan-targets card + M1.5 deck card + M1.6 support-card pool done (967 tests); M1.4 inheritance card + M1.7 coverage matrix next. Importer/data follow-on.** |
