...
| **M1 Inheritance workbench** | **🟧 active** | **from the 2026-06-25 handoff; built card-by-card M1.0–M1.8 (see section). M1.0–M1.3 (PR #12) + M1.5 deck (re-targeted from PR #11) on main, 947 tests; M1.4 (inheritance/parents) + M1.6 (support pool) are WIP draft branches. Importer/data follow-on.** |
