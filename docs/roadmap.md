...
| **M1 Inheritance workbench** | **🟧 active** | **from the 2026-06-25 handoff; built card-by-card M1.0–M1.8 (see section). On main: M1.0–M1.8 + M1.4b. Data gates CLOSED 2026-07-05: G1 saddle-id whitelist populated (win-bonus live) + wonRaces import confirmed shipped with M1.4. M1.4b rental builder + search-link DONE 2026-07-06 (Draft search-spec + deep-links, Rental manual veteran editor, `resolveParent2` wired into affinity + coverage). Remaining M1 work: the S1 base-relation-points refresh, then Plans 3–5 (roster migration, search-builder + compare, pedigree view).** |
