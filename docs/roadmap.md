...
| **M1 Inheritance workbench** | **🟧 active** | **from the 2026-06-25 handoff; built card-by-card M1.0–M1.8 (see section). M1.0 logic + M1.1 shell/route + M1.2 uma-plan card + M1.3 plan-targets card + M1.5 deck card + M1.6 support-card pool done (978 tests); M1.4 inheritance card + M1.7 coverage matrix next. Importer/data follow-on.** |
