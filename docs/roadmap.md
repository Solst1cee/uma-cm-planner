...
- 🟧 **M1.7 — "Obtainable vs. wishlist" card.** *Core done (2026-07-01), 2 data gates pending.* Coverage **matrix** (`Innate ‖ Parent | G.parent ‖ Chain | Random`, real per-member inherit-%, uncovered badge) + **bonus** list + Matrix/Coverage toggle. `greenSparkReconcile.ts` + `g1Saddle.ts` + `coverageMatrix.ts` + `CoverageMatrixCard` wired in `InheritancePage`. **Data-gated pending:** (1) G1 saddle ids (populate `data-overrides/g1_saddle_ids.json` + in-src copy → win-bonus goes live); (2) per-uma `innateSkills` pipeline (`scripts/borrowed/gametora/` sources → `pnpm data:build` → `umas.json` → full Innate column). Card degrades gracefully (0 win-bonus, unique-only Innate) until data arrives. **1151 tests.**
