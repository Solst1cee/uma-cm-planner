...
- 🟧 **M1.7 — "Obtainable vs. wishlist" card.** *Core done (2026-07-01), 2 data gates pending.* Coverage **matrix** (`Innate ‖ Parent | G.parent ‖ Hint | Chain | Random`, real per-member inherit-%, uncovered badge) + **bonus** list + Matrix/Coverage toggle. `greenSparkReconcile.ts` + `g1Saddle.ts` + `coverageMatrix.ts` + `CoverageMatrixCard` wired in `InheritancePage`. **Data-gated pending:** (1) G1 saddle ids (populate `data-overrides/g1_saddle_ids.json` + in-src copy → win-bonus goes live); (2) per-uma `innateSkills` pipeline (`scripts/borrowed/gametora/` sources → `pnpm data:build` → `umas.json` → full Innate column). Card degrades gracefully (0 win-bonus, unique-only Innate) until data arrives. **1154 tests.**
...
| **M1 Inheritance workbench** | **🟧 active** | **from the 2026-06-25 handoff; built card-by-card M1.0–M1.8 (see section). On main: M1.0–M1.6 (incl. M1.4 inheritance card + Star-Tracks picker + M1.6 support-card pool with unique/base effects, deck conflict rules, detail-card polish). M1.7 coverage matrix: core done (2026-07-01), 2 data gates pending (G1 saddle ids + innate-skill pipeline); 1154 tests. Next: M1.8 target-spark card.** |
