...
- ✅ **M1.3 — "Plan targets" card.** *Landed 2026-06-26.* Collapsible `cmp-plan-card` (left column): **blue** stat-spark goals (`sparkGoals.blue`) with `−/N★/+` steppers (clamped 0–9) + `×` delete + an add-stat select; **pink** aptitude sparks display-only (reuses `umaPlanAptChips` — aptitude + target grade); **wishlist** rows (skill name + SP, gold-tinted for gold rarity) under a "{n} skills · {SP} SP" headline. Pure helpers in `planTargets.ts` (TDD); presentational `PlanTargetsCard` wired to `setPlan` + gameData `skillById`. **882 tests.**
...
| **M1 Inheritance workbench** | **🟧 active** | **from the 2026-06-25 handoff; built card-by-card M1.0–M1.8 (see section). M1.0 logic + M1.1 shell/route + M1.2 uma-plan card + M1.3 plan-targets card done (882 tests); M1.4 (inheritance/parents card) next. Importer/data follow-on.** |
