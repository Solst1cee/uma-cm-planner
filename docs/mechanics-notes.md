...
✅ **CONFIRMED:** cumulative **10/20/30/35/40%** at hint Lv1–5 (increments `[10,10,10,5,5]`), cap 40%. JP wikis unanimous since launch (Altema/Game8/GameWith) + uma.guide. The umamusu.wiki "~8% per level" is **refuted** as a schedule — it's 40%/5 linearized. *Fast Learner* (切れ者) adds a further 10% (multiplicative ×0.9 in umalator's implementation).
