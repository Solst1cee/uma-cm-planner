...
- **(H) User version override, recorded on the plan** (user design, 2026-07-03). The app picks each skill's **default** version internally (`effectiveVersion` — confirmed `globalVer` at Current, horizon-arrived versions beyond). But when a patch is imminent, the user can **pin a specific version per skill in the UI**; the plan records it: `WishlistItem.skillVer?: number` (absent = default; the `uniqueSkillLevel` precedent — optional field, no Dexie bump, survives export/import). Version resolution everywhere becomes `item.skillVer ?? effectiveVersion(...).ver`. The picker lives on the wishlist plate in the planner sidebar (like the unique-level stepper), only for skills carrying `rebalance`; a pinned non-default version is visually marked (it's a deliberate what-if). In 4b, sims honor the pinned version's parameters.
