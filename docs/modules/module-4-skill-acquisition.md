...
- **Deferred on the new page (next):** the **Skill chart + Uma chart** (still on `/legacy`; vendor-umalator-chart-UI vs. reuse-`/legacy`-components decision pending), **card-hint sourcing**, and **HP / velocity / skill-activation zones** on the track (needs the engine's per-frame run trace, not just `getSimCourse`).
