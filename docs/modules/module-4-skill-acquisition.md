...
- **Skill-detail graphs** — **DONE 2026-06-17; rebuilt + polished 2026-06-20.** Rendered inside `SkillDetailDisclosure` when a `traceContext` is provided and the row is expanded. No external data: VFalator is an umalator fork, so everything was already computed inside our vendored bundle and just discarded. **Two views, both auto-run on expand:** (1) a **velocity-vs-time** curve (single representative run); (2) the umalator-style **"L gained by activation position"** + **"Activation frequency by position"** column pair, from N Monte-Carlo samples. *(Research note: umalator-global's `length-difference-chart.tsx` "Length Difference Impact" is the model — X = where the skill fires, Y = max バ身 from firing there. `alpha123/uma-skill-tools` (what VFalator uses) only has histogram/velocity matplotlib charts; mee1080 has a contribution table, not a chart.)*
  ...
  - **Pure SVG geometry** (`src/features/cm-planner/skill-trace/geometry.ts`): `vtPoints`/`activationTimes` (velocity + activation zone), `impactByPosition` (max バ身 per position bin), `frequencyByPosition` (% per bin), `binColumns`, `lAxisDomain` (integer-L), `gridLinesX/Y`, `distancePhaseBands`/`timePhaseBands` (**4 phases**: Early/Mid/Late/Last-spurt at 1/6, 2/3, 5/6). No chart library.
  - **Chart components** (`SkillTraceCharts.tsx` + `skill-trace.css`): `VelocityTimeChart`, `LengthImpactChart`, `ActivationFrequencyChart`, `RunChoiceToggle` (`Best | Typical | Worst`, re-renders with no re-sim). Square corners, faint 1L/500m grid, phase bands + labels matching the §0 racetrack colors, y-labels in a thin left gutter.
...
- **Deferred on the new page (next):** the **full-race uma1-vs-uma2 compare** (umalator's main view — both full builds + all wishlist skills, one run over the entire race, velocity + バ身 gap vs distance, activation markers; needs a uma2/rival build choice — explicitly deferred to a later session 2026-06-20); the **white/gold Skill chart** (still on `/legacy`; vendor-umalator-chart-UI vs. reuse-`/legacy`-components decision pending), **card-hint sourcing + wishlist**, and **HP / velocity / skill-activation zones** on the track (racetrack overlay; the skill-detail graphs are done but the racetrack's own activation-zone rendering needs the engine's per-frame run trace piped to the track SVG).
