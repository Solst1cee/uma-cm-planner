...
- **Skill-detail graphs (velocity / L-vs-distance / activation rate)** — **DONE 2026-06-17.** Rendered inside `SkillDetailDisclosure` when a `traceContext` is provided and the row is expanded. No external data: VFalator is an umalator fork, so the per-frame trace was already computed inside our vendored bundle and just discarded.
  - **Engine wrappers** (`src/sim/run.ts`): `runSkillTrace(build, race, skillId, nsamples)` pulls per-frame with/without traces from `runComparison.runData`; derives `min/max/median/mean` from the sorted `results` array (the engine returns no aggregate fields directly). `skillActivationRate(build, race, skillId, nsamples)` counts activations from `runSkillComparison.skillActivations`. Full-comparison `runData` carries per-frame traces for both runners; skill-comparison `runData` carries only `sk` activation logs — do not conflate. Both guard `spd > 0` + `isSimulatable`.
  - **Types** (`src/sim/types.ts`): `SkillTrace`, `SkillTraceRun`, `SkillFrame`, `RunChoice` (`min|median|max`), `SkillRate`, `SimulationRun`. Worker request kinds `skillTrace` / `skillRate`; `SimClient.skillTrace()` / `skillRate()` narrow responses by `res.kind`.
  - **Pure SVG geometry** (`src/features/cm-planner/skill-trace/geometry.ts`): `vtPoints` (velocity-vs-time), `gapCurve`/`activationTimes` (L-vs-distance + activation zone markers). No chart library.
  - **Chart components** (`src/features/cm-planner/skill-trace/SkillTraceCharts.tsx` + `skill-trace.css`): `VelocityTimeChart`, `LengthDistanceChart`, `ActivationRateBadge`, `RunChoiceToggle` (`Best | Typical | Worst`, re-renders with no re-sim).
  - **`useSkillTrace` hook** (`src/features/cm-planner/useSkillTrace.ts`): auto-fires curves on expand (`TRACE_SAMPLES = 20`); button-gated rate (`RATE_SAMPLES = 400`); `setRunChoice` switches display with no re-sim; sig key covers skill + course + full build stats.
  - **Wiring**: `SkillDetailDisclosure` accepts `traceContext` + `controlled-open` props; renders `SkillTraceSection` only when expanded and context is present. `UmaChartPanel` — accordion (one disclosure open at a time) + per-row `REFERENCE_STATS` `traceContext`. `PlannerSidebar` — plan-build `traceContext` on unique + wishlist rows.
...
- **Deferred on the new page (next):** the **white/gold Skill chart** (still on `/legacy`; vendor-umalator-chart-UI vs. reuse-`/legacy`-components decision pending), **card-hint sourcing + wishlist**, and **HP / velocity / skill-activation zones** on the track (racetrack overlay; skill-detail graphs are done but the racetrack's own activation-zone rendering needs the engine's per-frame run trace piped to the track SVG).
...
| Deferred | Blocked on |
|---|---|
| ~~Effect-summary badges~~ + ~~L-vs-distance / speed graphs~~ + ~~activation rate~~ + duration | **Effect-chips DONE 2026-06-16** (Codex `d043011`): effect/variant data in `skills.json` → `SkillDetailDisclosure`. **Skill-detail graphs DONE 2026-06-17** (`VelocityTimeChart`, `LengthDistanceChart`, `ActivationRateBadge` — `useSkillTrace` hook, `TRACE_SAMPLES=20`/`RATE_SAMPLES=400`). Duration labels still open. |
