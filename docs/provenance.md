...
- **Output layout + contract (`public/data/icons/`, git-tracked, generated — never hand-edit, P5):**
  ...
  - `ui/<id>.webp` — small UI glyphs (5 stat icons `stat-spd/sta/pow/gut/wit`, 8 aptitude-rank letters `apt-G…apt-S`, 5 mood icons). Aptitude/mood source: `utx_ico_statusrank_0n.png` / `global/utx_ico_motivation_m_0n.png` in the uma-tools dump (`UI_ICON_SOURCES` map). **The 5 stat icons are the in-game COLOURED tiles** (glyph on a colour square) — `utx_ico_obtain_00..04` sprites from the `atlas/common/common` bundle, extracted headlessly from the Global Steam client (decryption: RE'd from croakfang/UmaViewer — SQLite3MC `meta` + XOR `dat`, captured in the asset-acquisition memory). Vendored gitignored at `spikes/assets/stat-icons-colored/stat-*.png`; `build-icons.ts` OVERRIDES the white-glyph `status_0n.png` dump source with these (absent → white fallback, so CI keeps the committed coloured webps).
...
- **Support-card FULL ART (`public/data/card-art/<cardId>.webp`, web-sized 512px, committed; lives OUTSIDE `icons/`).** Source: `supportcard/support<id>/tex_support_card_<id>` Texture2D (2048²) extracted headlessly from the Global Steam client (decryption RE'd from croakfang/UmaViewer — SQLite3-Multiple-Ciphers `meta` + XOR `dat`; full method/keys in the `asset-acquisition-state` agent memory + the `umaextract.py` tool). The full-res 2048² PNGs are kept OUT of git (sibling `uma-cm-planner-assets/`); only the 512px WebP set is committed (~11 MB, 226 cards). `buildIcons` does NOT regenerate these (needs the encrypted client) — it only **indexes** the committed `card-art/` dir into the manifest's `cardArt[]`; the app resolves via `cardArtPath` in `src/core/icons.ts`. Cygames art, outside the GPL grant.
