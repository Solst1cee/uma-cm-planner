...
- **Local modification (2026-06-29):** unique-skill level scaling — each skill effect's modifier is scaled by `coef(ability_type, level) / 10000` before being applied. Formula: `effective_modifier = base_modifier × coef / 10000`. Coef table (`skill-level-coef.ts`) derived from master.mdb `skill_level_value`; schema of that table mirrored in `spikes/repos/umamusu-utils/misc/queries.txt` (retrieved 2026-06-29). Behavior corroborated by umareference.com unique-skills guide (recovery skills +2% per level; most others +1% at Lv2 then +3% per level thereafter). Lv1 coef = 10000 so Lv1/absent ≡ ×1.0 — fidelity 0.2202 unchanged at Lv1. Duration is NOT scaled (in-game behaviour). Source diff captured in `engine-patches/2026-06-29-skill-level-scaling.patch`; apply then `pnpm sim:build`. Spec: `docs/superpowers/specs/2026-06-29-m4-unique-skill-level-design.md`.
