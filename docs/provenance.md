...
- **Output layout + contract (`public/data/icons/`, git-tracked, generated — never hand-edit, P5):**
  ...
  - `uma/<umaId>.webp` (87). **Source: the game-native `trained_chr_icon_<charaId>_<iconAssetId>_02` texture extracted headlessly from the Global client** (vendored gitignored at `spikes/assets/uma-icons-game/<umaId>.png`). ⚠ The uma-tools dump version was horizontally **STRETCHED** (wide gold frame); the game native is correctly proportioned, so `build-icons.ts` prefers the game icon per umaId and falls back to the dump (`chara/trained_chr_icon_…_02.png`, else `chara/chr_icon_<charaId>.png`) only when the game vendor is absent. `charaId = floor(umaId/100)`; most `iconAssetId` = `umaId`, with 17 Global alt-outfit overrides in `scripts/build-icons.ts`.
