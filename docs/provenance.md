...
- **Support-card FULL ART (`public/data/card-art/<cardId>.webp`, web-sized **461×512**, committed; lives OUTSIDE `icons/`).** Source: `supportcard/support<id>/tex_support_card_<id>` Texture2D extracted headlessly from the Global Steam client (decryption RE'd from croakfang/UmaViewer — SQLite3-Multiple-Ciphers `meta` + XOR `dat`; full method/keys in the `asset-acquisition-state` agent memory + the `umaextract.py` tool). ⚠ The in-game texture is a **square 2048²** but the illustration is **~0.9 (taller-than-square)** — displayed 1:1 it looks horizontally stretched, so the web set is re-exported at **0.90 aspect (461×512)** to un-stretch. The full-res 2048² square masters are kept OUT of git (sibling `uma-cm-planner-assets/support-card-full-art/`); only the corrected 461×512 WebP set is committed (~9 MB, 226 cards). `buildIcons` does NOT regenerate these (needs the encrypted client) — it only **indexes** the committed `card-art/` dir into the manifest's `cardArt[]`; the app resolves via `cardArtPath` in `src/core/icons.ts`. Cygames art, outside the GPL grant.
