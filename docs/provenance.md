...
- **Output layout + contract (`public/data/icons/`, git-tracked, generated — never hand-edit, P5):**
  ...
  - `ui/<id>.webp` — small UI glyphs (5 stat icons `stat-spd/sta/pow/gut/wit`, 8 aptitude-rank letters `apt-G…apt-S`, 5 mood icons). Source: `status_0n.png` / `utx_ico_statusrank_0n.png` / `global/utx_ico_motivation_m_0n.png` in the uma-tools dump (`UI_ICON_SOURCES` map). ⚠ the stat glyphs are the bare white symbols (no coloured tile) — the in-game coloured stat tiles are a separate JP/Global CDN extraction (planned — alongside support-card full art).
