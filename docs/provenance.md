...
- **Output layout + contract (`public/data/icons/`, git-tracked, generated — never hand-edit, P5):**
  ...
  - `icon-manifest.json` = `{ dataVersion, format:"webp", skill:string[], card:string[], uma:string[], ui:string[], rank:string[], _fallbackUmas:string[] }`; the numeric id arrays are numeric-sorted, `ui`/`rank` in canonical label order; `dataVersion` = the same `global-…` the other builders use.
