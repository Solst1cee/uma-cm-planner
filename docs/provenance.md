...
**M1.4b encoding confirmation (2026-07-06):** `src/core/rentalSearch.ts` implements the three deep-link builders (`umaMoeUrl`/`pureDbUrl`/`chronoGenesisUrl`). No browser was available this session — the manual "does the filter rehydrate on the live site" check above is **still deferred to the human**; nothing below is live-verified, only source/doc-verified.
