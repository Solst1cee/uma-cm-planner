...
**Status (2026-06-17):** all four modules in progress on `main` (**517 tests**, typecheck + build green). **M4 is being rebuilt around umalator's _vendored_ UI** — the mockups are umalator-derived, so vendoring umalator's real components is the fastest path to fidelity (the hand-rolled track was far below the real uma-tools track that uma.guide ships). `/` is now `CmPlannerPage`: a **light CmPlan planner sidebar** with stat growth + pink-spark target summary + the **real umalator §0 race-track visualizer** + a **race-setup customizer** (preset ⇄ fully-custom track / ground / weather / season) + an engine-ranked **unique-skill chart** (VFalator-style sortable table; best-of-4-styles L on a fixed reference) + **skill effect-chips** in the detail viewer + **skill-detail graphs** (velocity-vs-time auto, plus umalator-style **length-gained-by-activation-position** + **activation-frequency-by-position** columns from N samples inside each `SkillDetailDisclosure`). The original engine-driven skill chart moved to **`/legacy`**. Rebuild spec: [2026-06-16-m4-umalator-build-foundation-design.md](docs/superpowers/specs/2026-06-16-m4-umalator-build-foundation-design.md).
...
|---|---|---|---|
| M4 Skill Acquisition | `/` (rebuild) · `/legacy` (old) | **Rebuild Slice A + sidebar + unique-skill chart done (2026-06-16); compact sidebar + skill-detail graphs done (2026-06-17):** CmPlan planner sidebar + selected-uma stat growth/base aptitude spark summary + vendored umalator §0 race-track + race-setup customizer + engine-ranked unique-skill chart + skill effect-chips + **velocity + length/frequency-by-activation-position graphs in skill disclosures** at `/`. Original engine skill chart + sourcing at `/legacy`. | [module-4](docs/modules/module-4-skill-acquisition.md) *(detailed)* |
...
- **Genuinely data-gated:** (1) skill effect-type/variants → **effect-chips DONE 2026-06-16** (effect/variant data baked into `skills.json`); velocity/L-vs-distance/activation-rate graphs → **DONE 2026-06-17** (`runSkillTrace`/`skillActivationRate`, `useSkillTrace` hook, SVG charts — no new data; VFalator is a umalator fork so the trace was already in the bundle); duration labels still open; (2) uma base aptitudes/stat growth → **DONE 2026-06-17** (`build-umas` bakes GameTora `aptitude` / `stat_bonus` into `umas.json`); innate skills + usable-here filters still open (M4/M1); (3) per-record release dates → Now/Upcoming/Future (M4); (4) banner + patch timeline entries → M3's empty lanes.
...
**M4 rebuild — next up (2026-06-17):** `/` now has the CmPlan planner sidebar (compact stats, stat growth, base-aptitude pink-spark summary), vendored umalator §0 race-track, race-setup customizer, the engine-ranked **unique-skill chart** (`UmaChartPanel` + `rankUmaChart`/`useUmaChart` — VFalator-style sortable table, run-on-demand, per-style L dropdown that re-ranks with no re-sim; `REFERENCE_STATS` all-1200 fixed reference, plan-independent), **skill effect-chips** (baked effect/variant data, `SkillDetailDisclosure`), and **skill-detail graphs** in each expanded `SkillDetailDisclosure`: an auto **velocity-vs-time** curve (`runSkillTrace` single representative run, Best/Typical/Worst toggle) plus an auto umalator-style **length-gained-by-activation-position** + **activation-frequency-by-position** pair (`skillImpact` → N-sample per-activation `{horseLength, positions}` binned by fire-position: max L/bin and %/bin; activation rate derived). Both auto-run on expand — velocity paints first (20 samples), the heavier impact pair (`IMPACT_SAMPLES`) queues behind it on the FIFO worker and paints when ready (~1s). Results are **memoized** in a module-level LRU keyed by sig (skill + build + course), so re-opening a skill (or Re-running the chart with the same build) is instant and never re-sims (`clearSkillTraceCache()` resets it). `useSkillTrace` hook, `TRACE_SAMPLES=20`/`IMPACT_SAMPLES=400`, hand-rolled SVG (no chart lib), phase bands + axis labels; wired in `UmaChartPanel` accordion + `PlannerSidebar`. Resume with: the **white/gold Skill chart** (vendor umalator's chart UI vs. reuse `/legacy` components — still open), **card-hint sourcing (§3)** + the uma **innate/release/usable-here columns** (base aptitudes/stat growth are already baked from `gametora/character-cards.json`), then **HP / velocity / skill-activation zones overlaid on the racetrack** (the skill-detail graphs are done; the racetrack SVG overlay still needs the per-frame trace piped to `RaceTrackView`). Specs/plans: [uma-chart](docs/superpowers/specs/2026-06-16-m4-uma-chart-design.md), [track](docs/superpowers/plans/2026-06-16-m4-slice-a-track.md), [sidebar](docs/superpowers/plans/2026-06-16-m4-planner-sidebar.md).
