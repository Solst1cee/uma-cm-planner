...
**Status (2026-06-16):** all four modules in progress on `main` (**478 tests**, typecheck + build green). **M4 is being rebuilt around umalator's _vendored_ UI** — the mockups are umalator-derived, so vendoring umalator's real components is the fastest path to fidelity (the hand-rolled track was far below the real uma-tools track that uma.guide ships). `/` is now `CmPlannerPage`: a **light CmPlan planner sidebar** + the **real umalator §0 race-track visualizer** + a **race-setup customizer** (preset ⇄ fully-custom track / ground / weather / season) + an engine-ranked **unique-skill chart** (VFalator-style sortable table; best-of-4-styles L on a fixed reference) + **skill effect-chips** in the detail viewer. The original engine-driven skill chart moved to **`/legacy`**. Rebuild spec: [2026-06-16-m4-umalator-build-foundation-design.md](docs/superpowers/specs/2026-06-16-m4-umalator-build-foundation-design.md).
...
- **Genuinely data-gated:** (1) skill effect-type/variants → **effect-chips DONE 2026-06-16** (effect/variant data baked into `skills.json`); duration → L-vs-distance graphs still open (M4/M2); (2) uma base stats/aptitudes/innate → uma base-aptitude column + innate sourcing — note the **unique-skill chart already shipped** without these (it ranks each uma's unique-skill L; aptitude bake from `gametora/character-cards.json` only gates the *base-aptitude column*) (M4/M1); (3) per-record release dates → Now/Upcoming/Future (M4); (4) banner + patch timeline entries → M3's empty lanes.
...
**M4 rebuild — next up (2026-06-16):** `/` now has the CmPlan planner sidebar, vendored umalator §0 race-track, race-setup customizer, the engine-ranked **unique-skill chart** (`UmaChartPanel` + `rankUmaChart`/`useUmaChart` — VFalator-style sortable table, run-on-demand, per-style L dropdown that re-ranks with no re-sim; `REFERENCE_STATS` all-1200 fixed reference, plan-independent), and **skill effect-chips** (baked effect/variant data, `SkillDetailDisclosure`). Resume with: the **white/gold Skill chart** (vendor umalator's chart UI vs. reuse `/legacy` components — still open), **card-hint sourcing (§3)** + the uma **base-aptitude / innate column** (bake from `gametora/character-cards.json` — already a `build-umas` input), then **HP / velocity / skill-activation zones** on the track (needs the engine's per-frame run trace / `chartData`, not just `getSimCourse`). Specs/plans: [uma-chart](docs/superpowers/specs/2026-06-16-m4-uma-chart-design.md), [track](docs/superpowers/plans/2026-06-16-m4-slice-a-track.md), [sidebar](docs/superpowers/plans/2026-06-16-m4-planner-sidebar.md).
