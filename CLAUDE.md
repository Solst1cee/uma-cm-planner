...
**M4 rebuild — next up (2026-06-20):** the primary planner surface, saved-plan inventory/JSON transfer, the engine-ranked **unique-skill chart** (`UmaChartPanel`/`rankUmaChart`/`useUmaChart`), the acquirable **white/gold skill chart** (`SkillChartPanel`/`rankSkillChart`), **skill effect-chips**, and the **skill-detail graphs** are live. The graphs render in each expanded `SkillDetailDisclosure`: an auto **velocity-vs-time** curve (Best/Typical/Worst) plus an auto umalator-style **length-by-activation-position** + **activation-frequency-by-position** pair (`skillImpact` → N-sample per-activation `{horseLength, positions}` binned by fire-position; activation rate derived). `useSkillTrace` hook, `TRACE_SAMPLES=20`/`IMPACT_SAMPLES=400`, **LRU-memoized** (instant re-open / Re-run), hand-rolled SVG with 4 phase bands + 1L/500m grid; wired in `UmaChartPanel` accordion + `PlannerSidebar` (both also honor main's `collapseSkillSignal`). Resume with: **card-hint sourcing (§3)**, the uma **innate/release/usable-here columns**, the **full-race uma1-vs-uma2 compare** ([handoff](docs/superpowers/plans/2026-06-20-m4-full-race-compare-handoff.md)), then **HP / velocity / skill-activation zones overlaid on the racetrack** (the skill-detail graphs are done; the racetrack SVG overlay still needs the per-frame trace piped to `RaceTrackView`). Handoffs: [module-4](docs/modules/module-4-skill-acquisition.md), [skill-detail graphs](docs/superpowers/plans/2026-06-20-m4-skill-detail-graphs-handoff.md), [inventory](docs/superpowers/plans/2026-06-18-m4-plan-inventory-card-handoff.md), [sidebar](docs/superpowers/plans/2026-06-18-m4-sidebar-polish-handoff.md), [track setup](docs/superpowers/plans/2026-06-18-m4-track-setup-card-handoff.md).
