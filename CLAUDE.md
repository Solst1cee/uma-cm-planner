...
**Status (2026-06-19):** all four modules in progress on `main` (**554 tests**, typecheck + build green). **M4 is rebuilt around umalator's _vendored_ UI.** `/` is now `CmPlannerPage`: a saved-plan inventory rail with JSON/ZIP transfer, a full-height light CmPlan sidebar with stat growth + pink-spark target summary, the real umalator §0 race-track visualizer, a preset/fully-custom race setup card, and engine-ranked unique + acquirable skill charts. The original engine-driven skill chart remains at `/legacy`. Rebuild spec: [2026-06-16-m4-umalator-build-foundation-design.md](docs/superpowers/specs/2026-06-16-m4-umalator-build-foundation-design.md).
...
|---|---|---|---|
| M4 Skill Acquisition | `/` (rebuild) · `/legacy` (old) | **Planner rebuild + inventory polish current through 2026-06-19:** saved-plan inventory/JSON transfer, full-height CmPlan sidebar, base-aptitude pink-spark summary, vendored track, custom race setup, unique-skill chart, and acquirable-skill L/SP/efficiency chart at `/`. Original engine skill chart + sourcing remain at `/legacy`. | [module-4](docs/modules/module-4-skill-acquisition.md) *(detailed)* |
...
**M4 rebuild — next up (2026-06-19):** the primary planner surface, inventory transfer workflow, unique-skill chart, and acquirable white/gold/inherited-skill chart are live. Resume with **card-hint sourcing (§3)**, Uma **innate/release/usable-here columns**, then **HP / velocity / skill-activation zones** on the track (requires the engine per-frame run trace / `chartData`). Current handoffs: [module-4](docs/modules/module-4-skill-acquisition.md), [inventory](docs/superpowers/plans/2026-06-18-m4-plan-inventory-card-handoff.md), [sidebar](docs/superpowers/plans/2026-06-18-m4-sidebar-polish-handoff.md), and [track setup](docs/superpowers/plans/2026-06-18-m4-track-setup-card-handoff.md). **Latest (2026-06-19, UNCOMMITTED on `main`'s working tree):** a chart-polish batch — Run→Stop button, persistent run-status + orange "Changed detected" stale prompt, italic captions, skill-chart rarity tabs (all/non-unique/inherited/white/gold via family×rarity reps), proc-aware "show not-activatable" (engine `activated` flag → `inactive` rows hidden+dimmed), sort-invert, and a uma "Rank by style" dropdown — plus a completed **force-activation (VFalator best-case) investigation** (feasible, but needs a `sim:build` rebuild). Resume here: [skill-chart polish + force-activation handoff](docs/superpowers/plans/2026-06-19-m4-skill-chart-polish-and-force-activation-handoff.md).
