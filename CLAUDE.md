...
**Status (2026-06-20):** all four modules in progress on `main` (**592 tests**, typecheck + build green). **M4 is rebuilt around umalator's _vendored_ UI.** `/` is now `CmPlannerPage`: a saved-plan inventory rail with JSON/ZIP transfer, a full-height light CmPlan sidebar with stat growth + pink-spark target summary, the real umalator §0 race-track visualizer, a preset/fully-custom race-setup card, engine-ranked **unique + acquirable** skill charts, skill **effect-chips** in the detail viewer, and **skill-detail graphs** inside each `SkillDetailDisclosure` (auto velocity-vs-time + umalator-style **length-by-activation-position** + **activation-frequency-by-position** columns from N samples). The original engine-driven skill chart remains at `/legacy`. Rebuild spec: [2026-06-16-m4-umalator-build-foundation-design.md](docs/superpowers/specs/2026-06-16-m4-umalator-build-foundation-design.md).
...
|---|---|---|---|
| M4 Skill Acquisition | `/` (rebuild) · `/legacy` (old) | **Planner rebuild + inventory + skill-detail graphs, current through 2026-06-20:** saved-plan inventory/JSON transfer, full-height CmPlan sidebar, base-aptitude pink-spark summary, vendored track, custom race setup, unique-skill chart, acquirable-skill L/SP/efficiency chart, and **velocity + length/frequency-by-activation-position graphs in skill disclosures** at `/`. Original engine skill chart + sourcing remain at `/legacy`. | [module-4](docs/modules/module-4-skill-acquisition.md) *(detailed)* |
...
**M4 rebuild — next up (2026-06-20):** the primary planner surface, saved-plan inventory/JSON transfer, the engine-ranked **unique-skill chart** (`UmaChartPanel`/`rankUmaChart`/`useUmaChart`), the acquirable **white/gold skill chart** (`SkillChartPanel`/`rankSkillChart`), **skill effect-chips**, and the **skill-detail graphs** are live. The graphs render in each expanded `SkillDetailDisclosure`: an auto **velocity-vs-time** curve (Best/Typical/Worst) plus an auto umalator-style **length-by-activation-position** + **activation-frequency-by-position** pair (`skillImpact` → N-sample per-activation `{horseLength, positions}` binned by fire-position; activation rate derived). `useSkillTrace` hook, `TRACE_SAMPLES=20`/`IMPACT_SAMPLES=400`, **LRU-memoized** (instant re-open / Re-run), hand-rolled SVG with 4 phase bands + 1L/500m grid; wired in `UmaChartPanel` accordion + `PlannerSidebar` (both also honor main's `collapseSkillSignal`). **Two-build race-sim compare + racetrack overlay DONE 2026-06-20** ([spec](docs/superpowers/specs/2026-06-20-m4-two-build-race-compare-track-overlay-design.md), [plan](docs/superpowers/plans/2026-06-20-m4-two-build-race-compare-track-overlay.md)): uma1 = active plan, uma2 = a chosen saved plan; `runRaceCompare` runs both full builds + all activations, returns velocity/HP traces + per-frame バ身 gap; `RaceOverlay` layers velocity/HP lines, activation markers, and the gap sub-band directly on the §0 racetrack SVG (`RaceTrackView` additive band: `ViewHeight=240` unchanged, `OverlayBandHeight` added only when a trace is present); `RaceComparePanel` with pluggable `Uma2Source` (savedPlan shipped; minusWishlist/reference are typed follow-ups); `useRaceCompare` hook with LRU memoization + Best/Typical/Worst toggle (no re-sim). ~~**HP / velocity / skill-activation zones overlaid on the racetrack**~~ — **DONE.** Resume with: **card-hint sourcing (§3)**, the uma **innate/release/usable-here columns**. Handoffs: [module-4](docs/modules/module-4-skill-acquisition.md), [skill-detail graphs](docs/superpowers/plans/2026-06-20-m4-skill-detail-graphs-handoff.md), [inventory](docs/superpowers/plans/2026-06-18-m4-plan-inventory-card-handoff.md), [sidebar](docs/superpowers/plans/2026-06-18-m4-sidebar-polish-handoff.md), [track setup](docs/superpowers/plans/2026-06-18-m4-track-setup-card-handoff.md). **Also merged 2026-06-19 (skill/unique chart polish):** Run→Stop button, persistent run-status + orange "Changed detected" stale prompt, italic captions, skill-chart 5 rarity tabs (all/non-unique/inherited/white/gold via family×rarity reps), proc-aware "show not-activatable" (`BashinStats.activated` from the engine's `skillActivations` → `inactive` rows hidden+dimmed), sort-invert, and a uma "Rank by style" dropdown. **Force-activation (VFalator "best-case")** was investigated and deferred — feasible but the exported `runSkillComparison` ignores forced params, so it needs a `sim:build` rebuild. Handoff: [skill-chart polish + force-activation](docs/superpowers/plans/2026-06-19-m4-skill-chart-polish-and-force-activation-handoff.md).
