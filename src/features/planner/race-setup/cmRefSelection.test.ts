...
describe('cmRefToSelection / selectionToCmRef', () => {
  ...
  it('round-trips a matched CM selection back to kind:cm (with geometry)', () => {
    const opts = cmRaceOptions(entries);
    const sel = cmRefToSelection({ kind: 'cm', cmId: 'CM15', cmNumber: 15, courseId: '10906', surface: 'turf', distance: 2200 }, catalog, entries);
    expect(selectionToCmRef(sel, opts)).toEqual({ kind: 'cm', cmId: 'CM15', cmNumber: 15, courseId: '10906', surface: 'turf', distance: 2200 });
  });
